...
- [x] T023 [US1] **Implemented** `apps/web/src/lib/publish/email.ts` (`publishEmail(issue,{test})`): recipients = active subscribers (`listActiveOrdered`) or admin-only in test; per-recipient idempotency via new `email_sends` table (skip status='sent', failures retriable); batches ≤50 + 1s pause; per-recipient try/catch; List-Unsubscribe + List-Unsubscribe-Post headers via `send.ts`; full RTL `issueEmailHtml` template. Wired into `run.ts` (email channel) + `/api/cron/publish` `{test:true}` (email-only, doesn't persist channel_results). Migration `supabase/migrations/0002_email_sends.sql` written (NOT applied). lint+build clean. **Gated: applying the migration + the real bulk send remain user-approved steps.**
