import type { Candidate, BrandVoice, Scored } from "./types";

export const CATEGORIES = [
  "ai",
  "business",
  "ecommerce",
  "automation",
  "tech",
  "marketing",
  "productivity",
] as const;

export const AUDIENCES = [
  "entrepreneurs",
  "store_owners",
  "marketers",
  "freelancers",
  "automation",
] as const;

// ── editorial notes (نصيحة الشاهين) — Editor's standing guidance, applied semantically ──
// Returns "" when there are no active notes so the prompts stay byte-identical to before (SC-003).
function notesGuidance(notes: string[]): string {
  if (!notes.length) return "";
  const list = notes.map((n, i) => `${i + 1}) ${n}`).join("\n");
  return `

نصائح المحرّر الثابتة (رأي «الشاهين» الشخصي — اعتمده لا الخبر إن تعارضا):
${list}
تعليمات هذه النصائح:
- إن انطبقت نصيحة منها فعلاً وبقوة على هذا الخبر تحديداً، اعكِس موقف المحرّر في «بعين الشاهين» (our_take) بنبرة النشرة.
- طبّق نصيحة واحدة كحدّ أقصى لكل خبر؛ وإن لم تنطبق أي نصيحة بوضوح فلا تُقحم شيئاً (الأصل: لا تطبيق).
- انقُل أي توصية أو رقم أو سعر أو اسم في النصيحة كما هو بأمانة تامة: لا تخفّفه ولا تعكسه ولا تتبنَّ الخيار المضادّ، حتى لو قال مصدر الخبر عكسه.`;
}

function voiceHeader(b: BrandVoice): string {
  return `أنت المحرّر الذكي لنشرة «${b.name}» — نشرة الذكاء الاصطناعي لروّاد الأعمال في الخليج.
جمهورك رائد أعمال صغير/متناهي الصغر/متوسط (SME) بميزانية وفريق محدودين، يريد أن يبني مشروعاً أو يحسّن عمله بالذكاء الاصطناعي — اربط كل خبر بفرصة أو أداة أو فكرة عملية يقدر يطبّقها بنفسه.
نبرتك: ${b.tone}
الأسلوب (مهم جداً):
- اكتب عربياً واضحاً بسيطاً، كأنك تحكي لصاحبك خبراً عملياً — لا فصحى متكلّفة ولا لغة تقارير جامدة. خاطب القارئ بـ«أنت».
- جُمل قصيرة وإيقاع حيّ؛ أحياناً سطر واحد قوي للتأكيد.
- حسّ فكاهي خفيف ولبق بين السطور (لمسة ذكية أو ملاحظة طريفة) — باعتدال، لا تُكثر، وابقَ موثوقاً ومهنياً.
- اشرح أي مصطلح تقني بكلمات سهلة فوراً (مثال: «وكيل ذكي» = برنامج ينفّذ مهامك خطوة بخطوة).
- تجنّب الكلمات المعقّدة أو المتكلّفة (مثل «تُديمقرط») واستخدم البديل الأبسط.
- بلا حشو ولا مبالغة تسويقية؛ كن محدّداً بالأرقام والأسماء والأسعار.
- ركّز دائماً على «ليش يهمك أنت كرائد أعمال» وكيف تطبّقها.`;
}

// ── classify + score (batched) ──
export function classifyScorePrompt(items: Candidate[]) {
  const system = `أنت محرّر نشرة عن «الذكاء الاصطناعي لروّاد الأعمال». جمهورك رائد أعمال صغير/متناهي الصغر/متوسط يريد أن يبني مشروعاً أو يحسّن عمله بالذكاء الاصطناعي — لا مستثمر ولا مهتم بأخبار الشركات الكبرى وصفقاتها. قيّم كل خبر من هذا المنظور حصراً. أعد JSON فقط بلا أي نص خارج JSON.`;
  const list = items
    .map(
      (it, i) =>
        `${i + 1}. id="${it.id}" | العنوان: ${it.title} | مقتطف: ${(it.content || "").slice(0, 400)}`
    )
    .join("\n");
  const user = `صنّف وقيّم كل خبر بعين «الشاهين» — نشرة ذكاء اصطناعي عملية لرائد أعمال يبني مشاريع ويستفيد من الذكاء الاصطناعي.
التصنيفات المسموحة (category_slug): ${CATEGORIES.join(", ")}
الجمهور المسموح (audience، اختر ما يناسب): ${AUDIENCES.join(", ")}

لكل خبر أعطِ:
- relevance (0-100): مدى صلته بالذكاء الاصطناعي + قابلية رائد الأعمال للاستفادة منه عملياً (فرصة/أداة/فكرة مشروع/تحسين عمل). خبر تقني عام لا علاقة له بالذكاء الاصطناعي أو ببناء عمل (أجهزة، ألعاب، سياسات، رسوم...) = منخفض جداً (أقل من 30). وأخبار التمويل والاستثمار والتقييمات والاكتتابات والاستحواذات وأرقام/صفقات الشركات الكبرى = relevance أقل من 20 (جمهورنا رائد أعمال صغير لا مستثمر)، إلا إذا أعطت الخبر أداة أو فرصة أو سعراً قابلاً للتطبيق مباشرة على مشروع صغير. وأخبار الطب والصحة (تشخيص، أدوية، علاجات، عيادات، أبحاث طبية) = relevance أقل من 25، إلا إذا قدّمت أداة أو فرصة قابلة للتطبيق مباشرة لرائد أعمال غير عامل في القطاع الصحي.
- importance (0-100): أهميته العملية لرائد أعمال يبني أو يستثمر بالذكاء الاصطناعي
- novelty (0-100): مدى جِدّته
- risk (0-100): احتمال أن يكون مبالَغاً فيه أو غير موثوق أو إشاعة
- is_tool (true/false): هل هو إطلاق أداة/منتج ذكاء اصطناعي جاهز للتجربة؟
- summary: جملة عربية واحدة تلخّص الخبر
- reason: كيف يستفيد منه رائد الأعمال عملياً (فرصة/أداة/فكرة)، أو لماذا لا يهم

الأخبار:
${list}

أعد بهذا الشكل بالضبط:
{"items":[{"id":"...","category_slug":"ai","audience":["entrepreneurs"],"relevance":80,"importance":70,"novelty":60,"risk":20,"is_tool":false,"summary":"...","reason":"..."}]}`;
  return { system, user };
}

// ── main story full treatment ──
export function mainStoryPrompt(b: BrandVoice, item: Candidate, summary: string, notes: string[] = []) {
  const system = `${voiceHeader(b)}
أنت تكتب «الانقضاضة» — الخبر الرئيسي لنشرة اليوم.
اكتب بدقّة زمنية: لا تصف منتجاً أو حدثاً معروفاً/قديماً كأنه جديد أو صدر «اليوم/أمس/للتو»؛ إن لم تتأكد من حداثته، صِفه دون إيحاء زمني خاطئ.
أعد JSON فقط.`;
  const user = `الخبر الرئيسي:
العنوان: ${item.title}
المحتوى: ${(item.content || summary).slice(0, 2500)}
الرابط: ${item.url ?? "غير متوفر"}

اكتب تحليلاً عمليّاً بحقول JSON التالية (كلها بالعربية):
- title: عنوان عربي جذّاب وموجز للخبر (لا تترك العنوان بالإنجليزية)
- what: ماذا حدث؟ (فقرة قصيرة واضحة)
- why: لماذا هذا مهم؟
- who: من يستفيد منه؟ (روّاد أعمال/تجار/مسوّقون/مستقلّون…)
- how: كيف يُطبَّق عمليّاً في السوق الخليجي؟ خطوة ملموسة
- warning: تحذير أو ملاحظة نقدية مختصرة (إن وُجد، وإلا اتركه فارغاً)
- our_take: رأي «${b.name}» التحريري المختصر — بصيرة لا يقدر المترجم الآلي يقلّدها${notesGuidance(notes)}

أعد: {"title":"...","what":"...","why":"...","who":"...","how":"...","warning":"...","our_take":"..."}`;
  return { system, user };
}

// ── roundup blurbs (batched) ──
export function roundupPrompt(
  b: BrandVoice,
  items: { id: string; title: string; summary: string }[],
  notes: string[] = []
) {
  const system = `${voiceHeader(b)}
أنت تكتب «رفّة جناح» — جولة أخبار سريعة. لكل خبر جملة أو جملتان عربيّتان تشرحان الخبر وأهميته بإيجاز.
اكتب بدقّة زمنية: لا تصف منتجاً أو حدثاً معروفاً/قديماً كأنه جديد أو صدر «اليوم/أمس/للتو»؛ إن لم تتأكد من حداثته، صِفه دون إيحاء زمني خاطئ.
أعد JSON فقط.`;
  const guidance = notesGuidance(notes);
  const takeField = guidance ? `,"our_take":"…(فقط إن انطبقت نصيحة على هذا الخبر، وإلا احذف الحقل)"` : "";
  const list = items.map((it) => `id="${it.id}" | ${it.title} — ${it.summary}`).join("\n");
  const user = `لكل خبر أعطِ عنواناً عربيّاً موجزاً (title) وفقرة قصيرة عملية (blurb، جملة أو جملتان) بنبرة النشرة:
${list}${guidance}

أعد: {"items":[{"id":"...","title":"...","blurb":"..."${takeField}}]}`;
  return { system, user };
}

// ── tools blurbs (batched) ──
export function toolsPrompt(
  b: BrandVoice,
  items: { id: string; title: string; summary: string }[],
  notes: string[] = []
) {
  const system = `${voiceHeader(b)}
أنت تكتب «عُدّة الشاهين» — أدوات/منتجات جديدة تستحق التجربة. لكل أداة: الاسم + جملة عملية عن فائدتها لرائد الأعمال. أعد JSON فقط.`;
  const guidance = notesGuidance(notes);
  const takeField = guidance ? `,"our_take":"…(فقط إن انطبقت نصيحة على هذه الأداة، وإلا احذف الحقل)"` : "";
  const list = items.map((it) => `id="${it.id}" | ${it.title} — ${it.summary}`).join("\n");
  const user = `لكل أداة أعطِ name (اسم مختصر) و blurb (جملة عربية عن الفائدة العملية):
${list}${guidance}

أعد: {"items":[{"id":"...","name":"...","blurb":"..."${takeField}}]}`;
  return { system, user };
}

// ── headline + intro + TL;DR ──
export function headlinePrompt(b: BrandVoice, scored: Scored[]) {
  const system = `${voiceHeader(b)}
أنت تكتب رأس النشرة: عنوان، افتتاحية «${b.name} يرصد لك اليوم…»، وملخص «نظرة الشاهين» (٥ نقاط).
اكتب بدقّة زمنية: لا تصف منتجاً أو حدثاً معروفاً/قديماً كأنه جديد أو صدر «اليوم/أمس/للتو»؛ إن لم تتأكد من حداثته، صِفه دون إيحاء زمني خاطئ.
أعد JSON فقط.`;
  const list = scored
    .slice(0, 12)
    .map((s) => `- ${s.summary}`)
    .join("\n");
  const user = `أهم أخبار اليوم:
${list}

أعطِ:
- title: عنوان جذّاب موجز لنشرة اليوم
- intro: افتتاحية قصيرة (٢-٣ جمل) تبدأ بروح «${b.name} يرصد لك اليوم…» وتربط خيط اليوم
- main_topic: الموضوع المحوري لليوم بكلمتين أو ثلاث
- tldr_bullets: مصفوفة من ٥ نقاط قصيرة جداً «وش صار اليوم» (كل نقطة سطر)

أعد: {"title":"...","intro":"...","main_topic":"...","tldr_bullets":["...","...","...","...","..."]}`;
  return { system, user };
}
