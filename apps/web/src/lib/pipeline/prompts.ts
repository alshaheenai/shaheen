...
جمهورك رائد أعمال يريد أن يبني مشروعاً أو يحسّن عمله بالذكاء الاصطناعي — اربط كل خبر بفرصة أو أداة أو فكرة عملية يقدر يطبّقها.
نبرتك: ${b.tone}
...
export function classifyScorePrompt(items: Candidate[]) {
  const system = `أنت محرّر نشرة عن «الذكاء الاصطناعي لروّاد الأعمال». جمهورك رائد أعمال يريد أن يبني مشروعاً أو يحسّن عمله بالذكاء الاصطناعي. قيّم كل خبر من هذا المنظور حصراً. أعد JSON فقط بلا أي نص خارج JSON.`;
  const list = items
    .map(
      (it, i) =>
...
لكل خبر أعطِ:
- relevance (0-100): مدى صلته بالذكاء الاصطناعي + قابلية رائد الأعمال للاستفادة منه عملياً (فرصة/أداة/فكرة مشروع/تحسين عمل). خبر تقني عام لا علاقة له بالذكاء الاصطناعي أو ببناء عمل (أجهزة، ألعاب، سياسات، رسوم...) = منخفض جداً (أقل من 30).
